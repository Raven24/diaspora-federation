...
class XmlPayload
  ...
  def self.unpack(xml)
    ...
    raise InvalidStructure unless valid_wrap?(xml)

    data = xml.nodes[0].nodes[0]
    klass_name = entity_class(data.name)
    raise UnknownEntity unless const_defined?(klass_name)

    const_get(klass_name).new(attribute_hash(data.nodes))
  end

  class InvalidStructure < RuntimeError; end
  class UnknownEntity < RuntimeError; end
end
