  ...
  module Salmon
    ...
    class MagicEnvelope
      ...
      def initialize(rsa_pkey, payload, parent_node = nil)
        fail ArgumentError unless rsa_pkey.instance_of?(OpenSSL::PKey::RSA) &&
                                  payload.is_a?(Entity)
      ...
      def self.unenvelop(magic_env, rsa_pubkey, cipher_params = nil)
        fail ArgumentError unless rsa_pubkey.instance_of?(OpenSSL::PKey::RSA) &&
                                  magic_env.instance_of?(Nokogiri::XML::Element)

        fail InvalidEnvelope unless envelope_valid?(magic_env)
        fail InvalidSignature unless signature_valid?(magic_env, rsa_pubkey)

        enc = magic_env.at_xpath('me:encoding').content
        alg = magic_env.at_xpath('me:alg').content

        fail InvalidEncoding unless enc == ENCODING
        fail InvalidAlgorithm unless alg == ALGORITHM
      ...
      # the given array should consist of the data, data_type (mimetype),
      # encoding and the algorithm
      # @param [Array<String>]
