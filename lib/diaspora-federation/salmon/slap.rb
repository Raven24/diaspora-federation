  ...
  module Salmon
    ...
    class Slap
      ...
      def entity(pubkey = nil)
        ...
        fail ArgumentError unless pubkey.instance_of?(OpenSSL::PKey::RSA)
      ...
      def self.from_xml(slap_xml)
        fail ArgumentError unless slap_xml.instance_of?(String)
        ...
        fail MissingAuthor if author_elem.nil? || author_elem.content.empty?
        slap.author_id = author_elem.content

        magic_env_elem = doc.at_xpath(magicenv_xpath, ns)
        fail MissingMagicEnvelope if magic_env_elem.nil?
      ...
      def self.generate_xml(author_id, pkey, entity)
        fail ArgumentError unless author_id.instance_of?(String) &&
